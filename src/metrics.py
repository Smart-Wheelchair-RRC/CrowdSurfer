...
class CalculateMetrics:
	...
	def __init__(self, odom_topic="/odom", collision_padding=0.5) -> None:
		...
		rospy.Subscriber("/pedsim_simulator/simulated_agents",
                   AgentStates, self.agent_states_callback)
	...
	def odom_callback(self, msg: Odometry) -> None:
		self.odom = msg
		self.position_array = np.append(
			self.position_array, [msg.pose.pose.position.x, msg.pose.pose.position.y])
		self.velocity_array = np.append(
			self.velocity_array, [msg.twist.twist.linear.x, msg.twist.twist.linear.y])
	...
	def get_path_length(self) -> float:
		...
		for i in len(self.position_array):
			if i == 0:
				continue
			path_length += np.linalg.norm(
				self.position_array[i] - self.position_array[i-1])
		return path_length
...
def main():
	...
	if rospy.is_shutdown():
		...
		print("3. Number of Collisions: ", metrics.num_collisions,
