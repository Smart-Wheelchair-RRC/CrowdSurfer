...
class LivePipeline(Pipeline):
    ...
    def _generate_occupancy_map_from_3d_lidar(
        ...
        # plt.imsave(
        #     "/home/laksh/crowdsurfer_ws/outputs/test/occu.png",
        #     occupancy_map,
        #     cmap="gray",
        # )
    ...
    def run(
        ...
        obstacle_positions_for_projection[
            :,
            self.max_projection_dynamic_obstacles : self.max_projection_dynamic_obstacles
            + point_cloud.shape[1],
        ] = point_cloud
        ...
        obstacle_positions_for_projection[
            :, 0 : dynamic_obstacles_n_steps.shape[-1]
        ] = dynamic_obstacles_n_steps[-1, 0:2, :]
        ...
        obstacle_velocities_for_projection[
            :, 0 : dynamic_obstacles_n_steps.shape[-1]
        ] = dynamic_obstacles_n_steps[-1, 2:, :]
        ...
        data = InferenceData(
            ...
            .type(torch.float32)
            .to(self.device),
            ego_velocity_for_projection=torch.from_numpy(ego_velocity)
            .unsqueeze(0)
            .type(torch.float32)
            .to(self.device),
            ...
            .type(torch.float32)
            .to(self.device),
            goal_position_for_projection=torch.from_numpy(goal)
            .unsqueeze(0)
            .type(torch.float32)
            .to(self.device),
            obstacle_positions_for_projection=torch.from_numpy(
                obstacle_positions_for_projection
            )
            .unsqueeze(0)
            .type(torch.float32)
            .to(self.device),
            obstacle_velocities_for_projection=torch.from_numpy(
                obstacle_velocities_for_projection
            )
            .unsqueeze(0)
            .type(torch.float32)
